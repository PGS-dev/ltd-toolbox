import type { GetFileComponentsResponse, GetFileComponentSetsResponse, PublishedComponent, PublishedComponentSet } from '@figma/rest-api-spec'
...
export async function getComponents(apiClient: FigmaApiInterface, fileId: string) {
  const components = await Promise.all([
    apiClient.withErrorDescriptions(errorDescriptions).request<GetFileComponentsResponse>(`files/${fileId}/components`).then(data => data.meta.components),
    apiClient.withErrorDescriptions(errorDescriptions).request<GetFileComponentSetsResponse>(`files/${fileId}/component_sets`).then(data => data.meta.component_sets)
