...
export const deepMerge = <First extends Obj, Second extends Obj>(first: First, second: Second): MergeDeep<First, Second> =>  {
  ...
  for (const key of Object.keys(second) as (keyof Second)[]) {
    ...
    if (key in second && Array.isArray(second[key]))
      second[key] = second[key].concat(output[key as keyof First]);

    // If object, then call deepMerge again
    if (second[key] as unknown instanceof Object)
      Object.assign(second[key], deepMerge(output[key as keyof First], second[key]));
  }
